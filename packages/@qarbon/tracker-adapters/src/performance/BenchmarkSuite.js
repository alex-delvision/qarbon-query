...
export class BenchmarkSuite {
    ...
    generateDatasets() {
        return [
            // Small datasets (< 1KB)
            {
                ...
                generator: () => Buffer.from(JSON.stringify({
            ...
            },
            {
                ...
                generator: () => Buffer.from('timestamp,model,emissions_kg,duration_seconds\n' +
            ...
            // Medium datasets (1KB - 100KB)
            {
                ...
                generator: () => {
                    ...
                    return Buffer.from(JSON.stringify({ records, metadata: { count: records.length } }));
                },
            },
            {
                name: 'medium-csv',
                ...
                generator: () => {
                    const headers = 'timestamp,model,emissions_kg,duration_seconds,energy_kwh,tokens_input,tokens_output';
                    const rows = Array.from({ length: 500 }, (_, i) => {
                        const hour = Math.floor(i / 60);
                        const minute = i % 60;
                        return (`2023-12-01T${String(hour).padStart(2, '0')}:${String(minute).padStart(2, '0')}:00Z,` +
                            `${i % 3 === 0 ? 'gpt-4' : 'gpt-3.5'},` +
            ...
            },
            {
                ...
                generator: () => {
                    const headers = 'id,timestamp,model,emissions_kg,duration_seconds,energy_kwh,tokens_input,tokens_output,region,provider,instance_type,cost_usd';
     ...
     */
    async benchmarkDataset(dataset) {
        this.forceGC();
        ...
        const earlyExitTriggered = this.registry.getLastEarlyExit?.() || false;
        this.forceGC();
        const memoryPeak = this.getMemoryUsageMB();
        // Handle both legacy and enhanced detection results
        const detectionResult = typeof result === 'string'
    ...
    async runFullSuite() {
        ...
        for (const dataset of datasets) {
            ...
            try {
                ...
                results.push(result);
                console.log(`   ✅ Detection time: ${result.metrics.detectionTimeMs}ms`);
                console.log(`   📏 Data size: ${(result.metrics.dataSizeBytes / 1024).toFixed(1)}KB`);
                console.log(`   🎯 Best match: ${result.result.bestMatch || 'none'}`);
                console.log(`   💾 Memory: ${result.metrics.memoryUsage.before}MB → ${result.metrics.memoryUsage.after}MB`);
                console.log(`   ⚡ Early exit: ${result.metrics.earlyExitTriggered ? 'Yes' : 'No'}`);
                console.log(`   🗄️  Cache hits: ${result.metrics.cacheStats.hits}, misses: ${result.metrics.cacheStats.misses}\n`);
            }
            catch (error) {
    ...
    generateReport() {
        ...
        // Summary statistics
        const avgTime = this.benchmarkResults.reduce((sum, r) => sum + r.metrics.detectionTimeMs, 0) / this.benchmarkResults.length;
        const maxTime = Math.max(...this.benchmarkResults.map(r => r.metrics.detectionTimeMs));
        const minTime = Math.min(...this.benchmarkResults.map(r => r.metrics.detectionTimeMs));
        ...
        report += '\n## Performance by Dataset Size\n\n';
        const sizeGroups = this.benchmarkResults.reduce((groups, result) => {
        ...
        for (const [size, results] of Object.entries(sizeGroups)) {
            const avgTime = results.reduce((sum, r) => sum + r.metrics.detectionTimeMs, 0) /
                results.length;
            const avgSize = results.reduce((sum, r) => sum + r.metrics.dataSizeBytes, 0) /
                results.length /
                1024;
    ...
    exportResults() {
        return JSON.stringify({
            ...
            summary: {
                ...
                averageTime: this.benchmarkResults.reduce((sum, r) => sum + r.metrics.detectionTimeMs, 0) / this.benchmarkResults.length,
                totalEarlyExits: this.benchmarkResults.filter(r => r.metrics.earlyExitTriggered).length,
                totalCacheHits: this.benchmarkResults.reduce((sum, r) => sum + r.metrics.cacheStats.hits, 0),
            },
