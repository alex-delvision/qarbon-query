...
export class OptimizedUniversalTrackerRegistry extends UniversalTrackerRegistry {
    ...
    async detectFormatOptimized(input) {
        ...
        const buffer = input instanceof Buffer
            ? input
            : await this.streamToBuffer(input);
        ...
        for (const [registryKey, adapter] of adaptersMap.entries()) {
            if (!confidenceResults.some(result => result.adapterName === registryKey)) {
        ...
        return {
            ...
            performance: {
                ...
                adapterTimings: adapterTimings.sort((a, b) => b.confidenceScore - a.confidenceScore),
    ...
    async warmupCache(sampleData) {
        ...
        console.log(`✅ Cache warmed up: ${stats.size} entries, ${stats.memoryUsageBytes} bytes`);
    }
}
