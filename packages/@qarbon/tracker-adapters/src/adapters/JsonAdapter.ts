...
export class JsonAdapter implements EmissionAdapter {
  /**
  ...
  detectConfidence(input: Buffer | NodeJS.ReadableStream): FormatConfidence {
    ...
    if (parsedData && typeof parsedData === 'object') {
      ...
      if (
        ('duration' in dataObj && 'emissions' in dataObj) ||
        ...
        const hasCodeCarbonFields = codeCarbonFields.some(
      ...
      if (Array.isArray(parsedData) && parsedData.length > 0) {
        ...
        if (typeof firstItem === 'object' && firstItem !== null) {
          ...
          if (parsedData.length > 1) {
            ...
            const similarStructure = parsedData.slice(1, 5).every(
              item =>
                typeof item === 'object' &&
                item !== null &&
