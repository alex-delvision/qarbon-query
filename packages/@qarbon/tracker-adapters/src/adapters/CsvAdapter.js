    ...
    detectConfidence(input) {
        ...
        if (lines.length < 2) {
            ...
            if (headerLine && headerLine.includes(',')) {
                ...
                const foundMappings = {
                    emissions: this.columnMapping.emissions.some(alias => headers.includes(alias.toLowerCase())),
                    duration: this.columnMapping.duration.some(alias => headers.includes(alias.toLowerCase())),
                    timestamp: this.columnMapping.timestamp.some(alias => headers.includes(alias.toLowerCase())),
                    model: this.columnMapping.model.some(alias => headers.includes(alias.toLowerCase())),
                };
        ...
        if (headerLine) {
            ...
            const reasonableHeaders = headers.filter(h => h.length > 1 && /^[a-zA-Z][a-zA-Z0-9_\-\s]*$/.test(h));
        ...
        if (sampleRows.length > 0) {
            const firstRowValues = sampleRows[0]?.split(',').map(v => v.trim().replace(/"/g, '')) || [];
            ...
            firstRowValues.forEach((_, colIndex) => {
                ...
                const numericValues = columnValues.filter(val => val !== '' && !isNaN(Number(val)));
        ...
        if (malformedCount > 0) {
            ...
            score *= penalty;
            evidence.push(`Malformed CSV structure detected (${malformedCount} issues)`);
        }
        return {
            adapterName: 'CsvAdapter',
    ...
    ingest(raw) {
        ...
        const data = lines.slice(1).map(line => {
            ...
            if (hasEmissionColumns) {
                ...
                const rowOut = {};
                if (mapIdx.ts !== undefined)
                    rowOut.timestamp = values[mapIdx.ts];
                if (mapIdx.model !== undefined)
                    rowOut.model = values[mapIdx.model];
