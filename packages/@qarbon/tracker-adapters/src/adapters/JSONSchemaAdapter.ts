...
export class JSONSchemaAdapter implements EmissionAdapter {
  private ajv: any; // eslint-disable-line @typescript-eslint/no-explicit-any
  private schemas: Record<string, JSONSchemaType<unknown>>;
  private strict: boolean;
  private lastDetectionResult: DetectionResult = { confidence: 0 };
  ...
  constructor(options: JSONSchemaAdapterOptions) {
    ...
    // Add format support
    (addFormats as any)(this.ajv); // eslint-disable-line @typescript-eslint/no-explicit-any
   ...
   */
  private score(data: unknown, schema: JSONSchemaType<unknown>): number {
   ...
   */
  ingest(raw: unknown): { data: unknown; schema: string; confidence: number } {
