...
export class FitAdapter {
    static FIT_HEADER_SIZE = 14;
    static FIT_FILE_TYPE_OFFSET = 8;
    static FIT_PROTOCOL_VERSION_OFFSET = 0;
    static FIT_PROFILE_VERSION_OFFSET = 4;
    static FIT_DATA_SIZE_OFFSET = 4;
    /**
    ...
    detectConfidence(input) {
        ...
        if (emissionMessageTypes.length > 0) {
            score += 0.1;
            evidence.push(`Found ${emissionMessageTypes.length} emission-related message types`);
        }
        return {
            adapterName: 'FitAdapter',
     ...
     */
    parseFitRecords(buffer, startOffset) {
        const records = [];
     ...
     */
    parseDefinitionMessage(buffer, offset) {
     ...
     */
    parseDataMessage(buffer, offset, messageType) {
     ...
     */
    findEmissionMessageTypes(buffer) {
        const emissionMessageTypes = [];
        ...
        for (let i = FitAdapter.FIT_HEADER_SIZE; i < buffer.length - 2; i++) {
            ...
            if (potentialEmissionTypes.includes(messageType) &&
...
//# sourceMappingURL=FitAdapter.js.map