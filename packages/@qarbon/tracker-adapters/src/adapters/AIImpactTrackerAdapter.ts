...
export class AIImpactTrackerAdapter implements EmissionAdapter {
  /**
    ...
    if (parsedData && typeof parsedData === 'object') {
      ...
      if (hasTokens) {
        ...
        if (tokensValue && typeof tokensValue === 'object') {
          const tokensObj = tokensValue as Record<string, unknown>;
  ...
  ingest(raw: unknown): {
    ...
    let input: any; // eslint-disable-line @typescript-eslint/no-explicit-any -- TODO: Type this more specifically
  ...
  private hasAIImpactTrackerFields(data: unknown): data is {
    ...
    return (
      ...
      'model' in data &&
      'tokens' in data &&
      (data as any).tokens && // eslint-disable-line @typescript-eslint/no-explicit-any
      typeof (data as any).tokens === 'object' && // eslint-disable-line @typescript-eslint/no-explicit-any
      'total' in (data as any).tokens && // eslint-disable-line @typescript-eslint/no-explicit-any
      'timestamp' in data &&
