...
export class JSONSchemaAdapter {
    ...
    detectConfidence(input) {
        ...
        if (bestSchemaScore === 1.0) {
            ...
            evidence.push(`Strong match with schema: ${bestSchemaMatch} (${(bestSchemaScore * 100).toFixed(1)}%)`);
        }
        else if (bestSchemaScore > 0.5) {
            evidence.push(`Partial match with schema: ${bestSchemaMatch} (${(bestSchemaScore * 100).toFixed(1)}%)`);
        }
        else if (bestSchemaScore > 0.2) {
            evidence.push(`Weak match with schema: ${bestSchemaMatch} (${(bestSchemaScore * 100).toFixed(1)}%)`);
        }
        else {
