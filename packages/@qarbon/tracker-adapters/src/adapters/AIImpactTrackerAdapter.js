...
export class AIImpactTrackerAdapter {
    ...
    detectConfidence(input) {
        ...
        if (parsedData && typeof parsedData === 'object') {
            ...
            if (requiredFieldCount === 4) {
                ...
                evidence.push(`Partial match (${requiredFieldCount}/4 required fields)`);
            }
            ...
            if ('emissions' in dataObj) {
                ...
                if (typeof emissionsValue === 'number' ||
    ...
    ingest(raw) {
        ...
        if (!this.hasAIImpactTrackerFields(input)) {
            throw new Error('AI Impact Tracker data must contain required fields: model, tokens.total, timestamp, energyPerToken');
        }
        ...
        if (isNaN(tokensTotal) || tokensTotal < 0) {
            throw new Error('AI Impact Tracker "tokens.total" must be a valid non-negative number');
        }
        ...
        if (tokens.prompt !== undefined &&
            ...
            throw new Error('AI Impact Tracker "tokens.prompt" must be a valid non-negative number');
        }
        if (tokens.completion !== undefined &&
            (isNaN(tokensCompletion) || tokensCompletion < 0)) {
            throw new Error('AI Impact Tracker "tokens.completion" must be a valid non-negative number');
        }
        ...
        if (isNaN(energyPerToken) || energyPerToken < 0) {
            throw new Error('AI Impact Tracker "energyPerToken" must be a valid non-negative number');
        }
        ...
        if (input.emissions !== undefined) {
            ...
            if (isNaN(emissions) || emissions < 0) {
                throw new Error('AI Impact Tracker "emissions" must be a valid non-negative number');
            }
        }
        else {
        ...
        if (input.confidence !== undefined) {
            ...
            if (typeof input.confidence === 'object' &&
                ...
                if (isNaN(confLow) || isNaN(confHigh)) {
                    throw new Error('AI Impact Tracker "confidence.low" and "confidence.high" must be valid numbers');
                }
                if (confLow > confHigh) {
                    throw new Error('AI Impact Tracker "confidence.low" must be less than or equal to "confidence.high"');
                }
                confidence = { low: confLow, high: confHigh };
            }
            else {
                throw new Error('AI Impact Tracker "confidence" must be an object with "low" and "high" properties');
            }
        }
        else {
