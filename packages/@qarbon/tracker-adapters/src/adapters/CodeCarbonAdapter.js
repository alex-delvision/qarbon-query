 ...
 */
export class CodeCarbonAdapter {
    /**
    ...
    detectConfidence(input) {
        ...
        if (parsedData && typeof parsedData === 'object') {
            ...
            else if ((hasDurationSeconds || hasDuration) &&
            ...
            if (optionalFieldCount > 0) {
                ...
                evidence.push(`${optionalFieldCount} optional CodeCarbon fields present`);
            }
    ...
    ingest(raw) {
        ...
        if (!this.hasCodeCarbonFields(input)) {
            throw new Error('CodeCarbon data must contain both "duration_seconds" and "emissions_kg" fields');
        }
        const durationSeconds = Number(input.duration_seconds); // eslint-disable-line @typescript-eslint/no-explicit-any
        const emissionsKg = Number(input.emissions_kg); // eslint-disable-line @typescript-eslint/no-explicit-any
...
//# sourceMappingURL=CodeCarbonAdapter.js.map