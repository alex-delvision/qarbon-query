...
export class CsvAdapter implements EmissionAdapter {
  private static readonly DEFAULT_COLUMN_MAPPING: ColumnMappingConfig = {
    timestamp: [
      'timestamp',
      'time',
      'date',
  ...
  private columnMapping: ColumnMappingConfig;
   ...
   */
  private safeNumber(value: string): number | undefined {
    if (!value || value.trim() === '') {
      return undefined;
    }
  ...
  setColumnMapping(mapping: Partial<ColumnMappingConfig>): void {
    Object.entries(mapping).forEach(([field, aliases]) => {
      if (aliases && field in this.columnMapping) {
        this.columnMapping[field as keyof ColumnMappingConfig] = aliases;
      }
    });
  }
