...
async function benchmarkBundleSizes() {
  ...
  const bundleTargets = [
    ...
    {
      name: 'Browser Bundle',
      file: 'qarbon-emissions.browser.min.js',
      target: '<45KB',
    },
  ];
  ...
  bundleTargets.forEach(({ name, file, target }) => {
    ...
    if (existsSync(filePath)) {
      ...
      const status =
        parseFloat(sizeKB) <= parseFloat(target.slice(1, -2)) ? '✅' : '❌';
...
async function benchmarkCalculations() {
  ...
  try {
    ...
    if (aiModule?.aiCalculator) {
      ...
      await benchmark.benchmark(
        'AI Batch Calculation (100 items)',
        async () => {
    ...
    if (cloudModule?.cloudCalculator) {
      ...
      await benchmark.benchmark('Cloud Compute Calculation', async () => {
        return cloudModule.cloudCalculator.calculateComputeEmissions(
          24,
          't3.micro'
        );
      });
      ...
      await benchmark.benchmark('Cloud Storage Calculation', async () => {
        return cloudModule.cloudCalculator.calculateStorageEmissions(
          100,
          's3-standard'
        );
      });
      ...
      await benchmark.benchmark(
        'Cloud Batch Calculation (100 items)',
        async () => {
          const inputs = Array(100)
            .fill(null)
            .map((_, i) => ({
    ...
    if (cryptoModule?.cryptoCalculator) {
      ...
      await benchmark.benchmark('Crypto Transaction Calculation', async () => {
        return cryptoModule.cryptoCalculator.calculateTransactionEmissions(
          10,
          'bitcoin'
        );
      });
      ...
      await benchmark.benchmark('Crypto Staking Calculation', async () => {
        return cryptoModule.cryptoCalculator.calculateStakingEmissions(
          1000,
          'ethereum',
          24
        );
      ...
      await benchmark.benchmark(
        'Crypto Batch Calculation (100 items)',
        async () => {
          const inputs = Array(100)
            .fill(null)
            .map((_, i) => ({
...
async function benchmarkMemoryUsage() {
  ...
  try {
    ...
    console.log(
      `    RSS: ${formatBytes(afterLoad.rss)} (+${formatBytes(afterLoad.rss - baseline.rss)})`
    );
    console.log(
      `    Heap Used: ${formatBytes(afterLoad.heapUsed)} (+${formatBytes(afterLoad.heapUsed - baseline.heapUsed)})`
    );
    console.log(
      `    Heap Total: ${formatBytes(afterLoad.heapTotal)} (+${formatBytes(afterLoad.heapTotal - baseline.heapTotal)})`
    );
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      if (modules[2]?.cryptoCalculator) {
        modules[2].cryptoCalculator.calculateTransactionEmissions(
          10,
          'bitcoin'
        );
      }
    ...
    console.log(
      `    RSS: ${formatBytes(afterCalc.rss)} (+${formatBytes(afterCalc.rss - afterLoad.rss)})`
    );
    console.log(
      `    Heap Used: ${formatBytes(afterCalc.heapUsed)} (+${formatBytes(afterCalc.heapUsed - afterLoad.heapUsed)})`
    );
    console.log(
      `    Heap Total: ${formatBytes(afterCalc.heapTotal)} (+${formatBytes(afterCalc.heapTotal - afterLoad.heapTotal)})`
    );
  } catch (error) {
...
async function generateReport(results) {
  ...
  for (const [name, target] of Object.entries(targets)) {
    ...
    if (result) {
      ...
      console.log(
        `  ${status} ${name.padEnd(35)} ${result.avg}ms (target: ${target}ms, ratio: ${ratio}x)`
      );
    }
  }
  ...
  if (singleAI && batchAI && singleAI > 0) {
    ...
    console.log(
      `  📦 Batch processing: ${batchSpeedup}x faster per item vs individual calculations`
    );
  }
