...
const { JsonAdapter, EmissionsCalculator, adapterRegistry } = require('@qarbon/emissions');
const fs = require('fs');
...
async function processJsonData() {
  ...
  try {
    ...
    console.log(`   Time range: ${jsonData.summary.time_range.start} to ${jsonData.summary.time_range.end}`);
    console.log(`   Total emissions: ${jsonData.summary.total_emissions_kg} kg\n`);
    ...
    console.log('🔄 Normalized data:', JSON.stringify(normalizedData, null, 2).substring(0, 400) + '...\n');
    ...
    for (let i = 0; i < jsonData.measurements.length; i++) {
      ...
      try {
        const result = await calculator.calculate(energyInput, {
          region: measurement.metadata.location.includes('west') ? 'US-WEST-1' : 'US-EAST-1',
        ...
        console.log(`   Input: ${measurement.metrics.energy.consumption_kwh} kWh`);
        console.log(`   Original: ${measurement.metrics.carbon.total_kg} kg CO2`);
        ...
        console.error(`❌ Error processing measurement ${measurement.id}:`, error.message);
      }
    }
    ...
    const calculatedTotal = allEmissions.reduce((sum, emission) => sum + emission.amount, 0) / 1000; // Convert g to kg
    ...
    calculationResults.forEach(result => {
      ...
      activityBreakdown[activity].total_kg += result.calculated_emissions.amount / 1000; // Convert g to kg
    });

    Object.entries(activityBreakdown).forEach(([activity, data]) => {
      console.log(`   ${activity}: ${data.count} measurements, ${data.total_kg.toFixed(3)} kg CO2`);
    });
