...
describe('AI Emissions Calculator', () => {
  ...
  describe('Multiple AI Models', () => {
    it('should calculate different emissions for different models', async () => {
      const gpt35Result = await calculator.calculateAIEmissions(
        1000,
        'gpt-3.5'
      );
      const gpt4Result = await calculator.calculateAIEmissions(1000, 'gpt-4');
      const claudeResult = await calculator.calculateAIEmissions(
        1000,
        'claude-2'
      );
    ...
    it('should handle fuzzy model name matching', async () => {
      ...
      const fuzzy1 = await calculator.calculateAIEmissions(
        1000,
        'gpt-3.5-turbo'
      );
      const fuzzy2 = await calculator.calculateAIEmissions(
        1000,
        'gpt-3.5-turbo-16k'
      );
