...
async function exampleBatchCalculation() {
  ...
  console.log(`  - Throughput: ${(metrics.totalInputs / (metrics.processingTime / 1000)).toFixed(0)} inputs/sec`);
...
async function exampleStreamingCalculation() {
  ...
  return new Promise<void>((resolve) => {
    ...
    streamingCalculator.on('data', (result) => {
      ...
      if (resultCount <= 5) {
        console.log(`📝 Result ${resultCount}: ${result.category} = ${result.amount} ${result.unit}`);
      }
...
async function exampleCachePerformance() {
  ...
  const { metrics: coldMetrics } = await batchCalculator.calculateBatch(testInputs);
  console.log(`  - Cache hits: ${coldMetrics.cacheHits}`);
  console.log(`  - Cache misses: ${coldMetrics.cacheMisses}`);
  console.log(`  - Processing time: ${coldMetrics.processingTime.toFixed(2)}ms`);
  ...
  const { metrics: warmMetrics } = await batchCalculator.calculateBatch(testInputs);
  console.log(`  - Cache hits: ${warmMetrics.cacheHits}`);
  console.log(`  - Cache misses: ${warmMetrics.cacheMisses}`);
  console.log(`  - Processing time: ${warmMetrics.processingTime.toFixed(2)}ms`);
...
function exampleFeatureFlags() {
  ...
  console.log(`  - enableWasmCalculations: ${scopedFlags.enableWasmCalculations}`);
  console.log(`  - wasmBatchThreshold: ${scopedFlags.wasmBatchThreshold}`);
  
  // Test feature checks
  console.log('✅ Feature checks:');
  console.log(`  - WASM enabled: ${featureFlags.isEnabled('enableWasmCalculations')}`);
  console.log(`  - SIMD enabled: ${featureFlags.isEnabled('enableSIMDOperations')}`);
  console.log(`  - Cache enabled: ${featureFlags.isEnabled('enableCache')}`);
  console.log(`  - WASM threshold: ${featureFlags.getValue('wasmBatchThreshold')}`);
}
...
function exampleLogParsing() {
  ...
  sampleLogs.forEach((log, index) => {
    ...
    if (result) {
      console.log(`    → ${result.category}/${result.type}: ${result.value} (${result.model || 'N/A'})`);
    } else {
