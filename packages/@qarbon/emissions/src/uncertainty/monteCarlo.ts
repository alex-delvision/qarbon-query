...
import { DistributionStats, UncertaintyRange, ParameterUncertainty, EmissionFunction } from './types';
...
export class RandomSampler {
   ...
   */
  static normal(mean: number, std: number, count: number, seed?: number): number[] {
   ...
   */
  static uniform(min: number, max: number, count: number, seed?: number): number[] {
    const random = seed ? this.seededRandom(seed) : Math.random;
    const samples: number[] = [];
   ...
   */
  static lognormal(mean: number, std: number, count: number, seed?: number): number[] {
   ...
   */
  static triangular(min: number, max: number, mode: number, count: number, seed?: number): number[] {
    const random = seed ? this.seededRandom(seed) : Math.random;
    const samples: number[] = [];
   ...
   */
  static fromUncertaintyRange(range: UncertaintyRange, count: number, seed?: number): number[] {
 ...
 */
export function calculateDistributionStats(samples: number[], ciLevel: number = 0.95): DistributionStats {
