...
export function exampleBasicMonteCarlo() {
  ...
  console.log(
    `  95% CI: [${result.confidenceInterval.low.toFixed(3)}, ${result.confidenceInterval.high.toFixed(3)}] g CO2e`
  );
...
export function exampleSensitivityAnalysis() {
  ...
  const emissionFn = (params: Record<string, number>) => {
    ...
    return (
      baseEmission * scalingFactor * efficiencyFactor + params.infrastructure
    );
  };
  ...
  results.forEach((result, index) => {
    ...
    console.log(
      `     First-order Sobol: ${(result.firstOrderSobol * 100).toFixed(1)}%`
    );
    console.log(`     Total Sobol: ${(result.totalSobol * 100).toFixed(1)}%`);
    console.log(
      `     Partial Derivative: ${result.partialDerivative.toFixed(6)}`
    );
  });
  console.log('');
...
export function exampleAIEmissionsWithUncertainty() {
  ...
  models.forEach(model => {
    ...
    confidenceLevels.forEach(level => {
      ...
      const result = calculator.calculateAIEmissionsWithUncertainty(
        1000,
        model,
        {
      ...
      if (result.uncertainty) {
        console.log(
          `  ${level}% CI: [${result.uncertainty.low.toFixed(2)}, ${result.uncertainty.high.toFixed(2)}] g CO2e (mean: ${result.uncertainty.mean.toFixed(2)})`
        );
      }
    });
  });
...
export function exampleUncertaintyPropagation() {
  ...
  const complexEmissionFn = (params: Record<string, number>) => {
    ...
    const trainingAmortization =
      params.trainingCost / params.modelLifetime / params.queriesPerDay;
    const infrastructureOverhead =
      params.datacenterPUE * params.baseInfrastructure;
  ...
  console.log(
    `  95% CI: [${linearResult.low.toFixed(3)}, ${linearResult.high.toFixed(3)}] g CO2e`
  );
  ...
  console.log(
    `  95% CI: [${mcResult.low.toFixed(3)}, ${mcResult.high.toFixed(3)}] g CO2e`
  );
...
export function exampleCompareUncertaintyMethods() {
  ...
  sampleSizes.forEach(size => {
    ...
    console.log(
      `  ${size} samples: mean=${result.mean.toFixed(2)}, range=${range.toFixed(2)}`
    );
  });
