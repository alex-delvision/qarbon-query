...
export class EnergyUsageAdapter extends BaseAdapter<EnergyUsageData> {
  constructor() {
    super({
      ...
      version: '1.0.0',
      description: 'Adapter for generic energy usage data from watt-meters and energy monitoring devices',
  ...
  validate(input: EnergyUsageData): ValidationResult {
    ...
    if (!input.energy) {
      ...
      const hasEnergyValue = input.energy.total_kwh !== undefined || input.energy.energy_kwh !== undefined;
      const hasPowerValue = input.energy.average_power_w !== undefined || 
                           input.energy.power_watts !== undefined ||
                           input.energy.peak_power_w !== undefined;
      
      if (!hasEnergyValue && !hasPowerValue) {
        errors.push('Energy object must contain either energy (kWh) or power (W) measurements');
      }
      ...
      if (input.energy.min_power_w !== undefined && input.energy.peak_power_w !== undefined) {
    ...
    if (input.measurement_period) {
      if (input.measurement_period.start && !this.isValidTimestamp(input.measurement_period.start)) {
        errors.push('Invalid measurement_period.start timestamp format');
      }
      if (input.measurement_period.end && !this.isValidTimestamp(input.measurement_period.end)) {
      ...
      if (input.measurement_period.start && input.measurement_period.end) {
        ...
        if (endDate <= startDate) {
          warnings.push('Measurement period end time should be after start time');
        }
      }

      if (input.measurement_period.duration_seconds !== undefined && input.measurement_period.duration_seconds < 0) {
  ...
  normalize(input: EnergyUsageData): NormalizedData {
    ...
    if (!validation.isValid) {
      throw new Error(`Invalid Energy Usage data: ${validation.errors?.join(', ')}`);
    }
    ...
    } else if (input.measurement_period?.start && input.measurement_period?.end) {
      const startDate = new Date(input.measurement_period.start);
      const endDate = new Date(input.measurement_period.end);
    ...
    return {
      ...
      timing: {
        ...
        measurement_start: input.measurement_period?.start ? 
          new Date(input.measurement_period.start).toISOString() : undefined,
        measurement_end: input.measurement_period?.end ? 
          new Date(input.measurement_period.end).toISOString() : undefined,
      ...
      emissions: input.metadata?.carbon_intensity && (energyValue || estimatedEnergy) ? {
        total: ((energyValue || estimatedEnergy || 0) * input.metadata.carbon_intensity) / 1000, // Convert to kg
        unit: 'kg',
        scope: 'scope2',
        estimated: true
      } : undefined,
    ...
    return [
      ...
      },
      {
        ...
        test: (data: any) => {
          ...
          if (typeof data !== 'object' || data === null) return false;
          const timeFields = ['timestamp', 'measurement_period', 'duration_minutes'];
