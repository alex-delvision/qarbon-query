...
const PUBLISHED_AI_FACTORS = {
  ...
  'claude-2': {
    co2PerToken: 0.0030, // g CO2/token (Anthropic estimates)
...
describe('Factor Sanity Checks', () => {
  describe('AI Model Factors', () => {
    describe('GPT-3.5 Factor Validation', () => {
      ...
      it('should have confidence intervals within reasonable bounds', () => {
        ...
        expectFactorTolerance(factor!.confidence.high, published.confidence.high);
        ...
        const meanValue = (factor!.confidence.high + factor!.confidence.low) / 2;
    ...
    describe('GPT-4 Factor Validation', () => {
      ...
      it('should emit more than GPT-3.5 due to increased model size', () => {
        ...
        expect(gpt4Factor!.co2PerToken).toBeGreaterThan(gpt35Factor!.co2PerToken);
        expect(gpt4Factor!.co2PerQuery).toBeGreaterThan(gpt35Factor!.co2PerQuery);
        expect(gpt4Factor!.energyPerToken).toBeGreaterThan(gpt35Factor!.energyPerToken);
      });
    });
  ...
  describe('Grid Intensity Factors', () => {
    ...
    it('should validate US regional grid intensities against EPA eGRID data', () => {
      ...
      const testRegions = [
        { region: 'california', expected: PUBLISHED_GRID_FACTORS.us_california },
        { region: 'texas', expected: PUBLISHED_GRID_FACTORS.us_texas },
        { region: 'virginia', expected: PUBLISHED_GRID_FACTORS.us_virginia },
        { region: 'washington', expected: PUBLISHED_GRID_FACTORS.us_washington }
      ];
    ...
    it('should have global average within published ranges', () => {
      ...
      expectFactorTolerance(globalAverage, PUBLISHED_GRID_FACTORS.global_average);
    });
  });
  ...
  describe('PUE Factor Validation', () => {
    it('should validate cloud provider PUE values against published data', () => {
      ...
      const testPUE = [
        { provider: 'aws_us_east_1', expected: PUBLISHED_PUE_VALUES.aws_us_east_1 },
        { provider: 'aws_us_west_2', expected: PUBLISHED_PUE_VALUES.aws_us_west_2 },
        { provider: 'gcp_us_central1', expected: PUBLISHED_PUE_VALUES.gcp_us_central1 }
      ];
