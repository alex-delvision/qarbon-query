...
import {
  monteCarlo,
  simpleMonteCarloRange,
} from '../../uncertainty/monteCarlo';
...
function calculateKolmogorovSmirnovStatistic(
  sample1: number[],
  sample2: number[]
): number {
...
const nonlinearFunction = (params: Record<string, number>) => {
  return (
    params.a * Math.pow(params.x, 2) +
    params.b * Math.sin(params.c * params.x) +
    params.d
  );
};
...
describe('Monte Carlo Convergence Tests', () => {
  describe('Sample Size Convergence', () => {
    it('should converge to theoretical mean with increasing sample size', () => {
      ...
      for (let i = 0; i < sampleSizes.length; i++) {
        ...
        console.log(
          `Sample size ${sampleSizes[i]}: mean=${result.mean.toFixed(3)}, error=${error.toFixed(3)}`
        );
    ...
    it('should converge to theoretical standard deviation with increasing sample size', () => {
      ...
      for (let i = 0; i < sampleSizes.length; i++) {
        ...
        console.log(
          `Sample size ${sampleSizes[i]}: std=${result.std.toFixed(3)}, error=${error.toFixed(3)}`
        );
    ...
    it('should have confidence intervals that stabilize with sample size', () => {
      ...
      const results = [1000, 5000, 10000].map(sampleSize =>
        monteCarlo(params => params.x, uncertainties, sampleSize, 0.95)
      );

      // Confidence interval width should decrease (or at least not increase significantly)
      for (let i = 1; i < results.length; i++) {
        const prevWidth =
          results[i - 1].confidenceInterval.high -
          results[i - 1].confidenceInterval.low;
        const currWidth =
          results[i].confidenceInterval.high -
          results[i].confidenceInterval.low;

        console.log(
          `CI width: ${prevWidth.toFixed(3)} → ${currWidth.toFixed(3)}`
        );
  ...
  describe('Distribution Convergence', () => {
    it('should produce samples that follow the expected distribution', () => {
      ...
      const result = monteCarlo(params => params.x, uncertainties, 10000, 0.95);
  ...
  describe('Reproducibility and Stability', () => {
    it('should produce similar results across multiple runs', () => {
      ...
      console.log(
        `Mean stability: ${meanOfMeans.toFixed(3)} ± ${stdOfMeans.toFixed(3)}`
      );
      console.log(
        `Std stability: ${meanOfStds.toFixed(3)} ± ${stdOfStds.toFixed(3)}`
      );
    ...
    it('should show convergence for nonlinear functions', () => {
      ...
      for (let i = 1; i < results.length; i++) {
        ...
        const meanChange =
          Math.abs(currResult.mean - prevResult.mean) /
          Math.abs(prevResult.mean);
        console.log(
          `Mean change (${sampleSizes[i - 1]} → ${sampleSizes[i]}): ${(meanChange * 100).toFixed(2)}%`
        );
  ...
  describe('Emissions-Specific Convergence', () => {
    it('should converge for realistic emissions calculations', () => {
      ...
      results.forEach((result, i) => {
        ...
        console.log(
          `Sample size ${sampleSizes[i]}: emissions=${result.mean.toFixed(4)} kg, error=${error.toFixed(4)}`
        );
    ...
    it('should handle multiple emission factors with convergence', () => {
      ...
      const factors = [0.0022, 0.0085, 0.003];
      ...
      models.forEach((model, index) => {
        ...
        const intervalWidth =
          result.confidenceInterval.high - result.confidenceInterval.low;
  ...
  describe('Statistical Validation', () => {
    it('should pass Kolmogorov-Smirnov test for distribution consistency', () => {
      ...
      console.log(
        `KS statistic: ${ksStatistic.toFixed(4)}, critical value: ${criticalValue.toFixed(4)}`
      );
    ...
    it('should maintain proper confidence interval coverage', () => {
      ...
      for (let i = 0; i < numTrials; i++) {
        ...
        if (
          trueValue >= result.confidenceInterval.low &&
  ...
  describe('Convergence Diagnostics', () => {
    it('should show decreasing variance of sample mean with sample size', () => {
      ...
      sampleSizes.forEach(size => {
        const means = Array.from({ length: 50 }, () => {
          return monteCarlo(params => params.x, uncertainties, size, 0.95).mean;
        });
        ...
        console.log(
          `Sample size ${size}: variance of means = ${variance.toFixed(6)}`
        );
      });
      ...
      for (let i = 1; i < meanVariances.length; i++) {
        ...
        console.log(
          `Variance ratio: ${ratio.toFixed(3)}, expected: ${expectedRatio.toFixed(3)}`
        );
    ...
    it('should demonstrate Central Limit Theorem convergence', () => {
      ...
      const sampleMeans = Array.from({ length: numTrials }, () => {
        return monteCarlo(params => params.x, uncertainties, sampleSize, 0.95)
          .mean;
      });
      ...
      const skewness =
        sampleMeans.reduce((sum, x) => sum + Math.pow((x - mean) / std, 3), 0) /
        numTrials;
      const kurtosis =
        sampleMeans.reduce((sum, x) => sum + Math.pow((x - mean) / std, 4), 0) /
        numTrials;

      console.log(
        `Sample means - Skewness: ${skewness.toFixed(3)}, Kurtosis: ${kurtosis.toFixed(3)}`
      );
