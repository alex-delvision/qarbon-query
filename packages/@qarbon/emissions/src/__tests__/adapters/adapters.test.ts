...
const codeCarbonSample = JSON.parse(readFileSync(join(fixturesPath, 'codecarbon-sample.json'), 'utf-8'));
const csvSampleContent = readFileSync(join(fixturesPath, 'csv-sample.csv'), 'utf-8');
const jsonSample = JSON.parse(readFileSync(join(fixturesPath, 'json-sample.json'), 'utf-8'));
...
describe('Adapter System', () => {
  ...
  describe('CodeCarbon Adapter', () => {
    ...
    describe('Validation', () => {
      ...
      it('should warn about data quality issues', () => {
        ...
        expect(result.warnings).toContain('Geographic coordinates appear to be invalid');
      });
    });
    ...
    describe('Normalization', () => {
      ...
      it('should throw error for invalid data during normalization', () => {
        ...
        expect(() => adapter.normalize(invalidData)).toThrow('Invalid CodeCarbon data');
      });
    });
  ...
  describe('CSV Adapter', () => {
    ...
    describe('Validation', () => {
      it('should validate CSV data with proper structure', () => {
        const csvData = {
          headers: ['timestamp', 'co2_kg', 'energy_kwh', 'power_w', 'duration_h', 'source', 'location'],
          rows: csvSampleContent.split('\n').filter(line => line.trim()).map(line => line.split(',')),
          config: {
            columnMapping: {
              timestamp: 'timestamp',
              emissions: 'co2_kg',
      ...
      it('should detect missing column mappings', () => {
        ...
        const result = adapter.validate(csvData);
        expect(result.warnings).toContain('No timestamp column mapping specified');
        expect(result.warnings).toContain('No emissions or energy column mapping specified');
      });
    });

    describe('Normalization', () => {
      it('should normalize CSV data correctly', () => {
        const rows = csvSampleContent.split('\n').filter(line => line.trim()).map(line => line.split(','));
        const csvData = {
          rows,
          config: {
            hasHeader: true,
  ...
  describe('Adapter Registry', () => {
    ...
    it('should unregister adapters', () => {
      ...
      const unregistered = adapterRegistry.unregisterAdapter('CodeCarbonAdapter');
