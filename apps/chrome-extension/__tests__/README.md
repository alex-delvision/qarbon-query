...
This directory contains comprehensive unit tests for the QarbonQuery Chrome extension token extraction and monitoring functionality.
...
**Status:** These tests need adjustment because the current streaming parser implementation in the tokenExtractors expects different formats than what the tests provide.
...
### Custom Test Data
All tests use realistic API response data based on actual provider APIs to ensure accuracy and reliability.
...
The test suite provides excellent coverage of the core functionality needed for the Chrome extension to track AI token usage accurately across multiple providers.
