(function () {
  ...
  window.fetch = async function (...args) {
    ...
    else if (patterns.chatgpt.some(p => url && url.includes(p)))
      platform = 'ChatGPT';
    else if (patterns.gemini.some(p => url && url.includes(p)))
      platform = 'Gemini';
        ...
        const stream = new ReadableStream({
          async start(controller) {
            try {
              ...
              console.log(
                `✅ Captured ${platform}: ${tokens} tokens, ${emissions.toFixed(4)} g CO₂e`
              );
            } catch (e) {
