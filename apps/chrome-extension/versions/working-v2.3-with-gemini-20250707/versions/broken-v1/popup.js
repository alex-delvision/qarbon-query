(function () {
      ...
      const results = await chrome.scripting.executeScript({
        ...
        func: () => {
          ...
          const captures = JSON.parse(
            localStorage.getItem('qarbon_sse_captures') || '[]'
          );
          ...
          return {
            today: {
              ...
              emissions: todayCaptures.reduce(
                (sum, c) => sum + (c.emissions || 0),
                0
              ),
              tokens: todayCaptures.reduce(
  ...
  function calculatePeriodStats(captures, days) {
    ...
    const periodCaptures = captures.filter(
    ...
    try {
      ...
      if (tab.id) {
        const results = await chrome.scripting.executeScript({
          target: { tabId: tab.id },
          func: () =>
            JSON.parse(localStorage.getItem('qarbon_sse_captures') || '[]'),
        });
  ...
  function updateEmissionsDisplay(period, stats) {
    ...
    const emissionsElement = document.getElementById(
      `emissions-total${suffix}`
    );
    if (emissionsElement) {
