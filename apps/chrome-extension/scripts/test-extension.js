...
if (!fs.existsSync(extensionDir)) {
  console.error(
    '❌ Extension directory not found. Run "npm run build:complete" first.'
  );
  process.exit(1);
}
...
for (const jsFile of jsFiles) {
  try {
    ...
    if (content.includes('import ') && !content.includes('export')) {
      console.warn(
        `⚠️  ${jsFile} contains import statements - may cause issues in Chrome extension`
      );
    }
...
console.log(
  `   📊 Background script: ${(metrics.backgroundSize / 1024).toFixed(2)} KB`
);
console.log(
  `   📊 Content script: ${(metrics.contentSize / 1024).toFixed(2)} KB`
);
console.log(`   📊 Popup script: ${(metrics.popupSize / 1024).toFixed(2)} KB`);
console.log(`   📊 Popup HTML: ${(metrics.htmlSize / 1024).toFixed(2)} KB`);

const totalJSSize =
  metrics.backgroundSize + metrics.contentSize + metrics.popupSize;
