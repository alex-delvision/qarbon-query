...
function check(description, testFn) {
  ...
  try {
    ...
    if (result.success) {
      ...
      failedChecks++;
      issues.push({ description, error: result.error, suggestion: result.suggestion });
    }
  } catch (error) {
    console.log(`❌ ${description} - Exception: ${error.message}`);
    failedChecks++;
    issues.push({ description, error: error.message, suggestion: 'Check code syntax and file access' });
  }
}
...
check('Extension directory exists', () => {
  ...
  return { success: false, error: 'Extension directory not found', suggestion: 'Run npm run build:extension' };
});

check('Manifest.json exists and is valid', () => {
  const manifestPath = path.join(EXTENSION_DIR, 'manifest.json');
  if (!fs.existsSync(manifestPath)) {
    return { success: false, error: 'manifest.json not found', suggestion: 'Copy manifest.json to extension directory' };
  }
  
  try {
    const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
    if (manifest.manifest_version !== 3) {
      return { success: false, error: `Wrong manifest version: ${manifest.manifest_version}`, suggestion: 'Update to manifest_version: 3' };
    }
    return { success: true };
  } catch (e) {
    return { success: false, error: 'Invalid JSON in manifest.json', suggestion: 'Fix JSON syntax errors' };
  }
});
...
check('Background script exists', () => {
  ...
  if (!fs.existsSync(backgroundPath)) {
    return { success: false, error: 'background.js not found', suggestion: 'Check webpack build output' };
  }
  
  const stats = fs.statSync(backgroundPath);
  if (stats.size === 0) {
    return { success: false, error: 'background.js is empty', suggestion: 'Check TypeScript compilation' };
  }
...
check('Content script exists', () => {
  ...
  if (!fs.existsSync(contentPath)) {
    return { success: false, error: 'content.js not found', suggestion: 'Check webpack build output' };
  }
  return { success: true };
});
...
check('DNR rules file exists and is valid', () => {
  ...
  if (!fs.existsSync(dnrPath)) {
    return { success: false, error: 'dnr_rules.json not found', suggestion: 'Copy dnr_rules.json to extension directory' };
  }
  
  try {
    const rules = JSON.parse(fs.readFileSync(dnrPath, 'utf8'));
    if (!Array.isArray(rules) || rules.length === 0) {
      return { success: false, error: 'DNR rules array is empty or invalid', suggestion: 'Add valid DNR rules' };
    }
    return { success: true };
  } catch (e) {
    return { success: false, error: 'Invalid JSON in dnr_rules.json', suggestion: 'Fix JSON syntax' };
  }
});
  ...
  for (const api of deprecatedAPIs) {
    if (content.includes(api)) {
      return { 
        ...
        suggestion: 'Remove all webRequest API usage and use declarativeNetRequest' 
      };
    }
  }
...
check('Manifest uses service_worker (not background scripts)', () => {
  ...
  if (manifest.background?.scripts) {
    return { success: false, error: 'Uses deprecated background.scripts', suggestion: 'Use background.service_worker' };
  }
  
  if (!manifest.background?.service_worker) {
    return { success: false, error: 'Missing background.service_worker', suggestion: 'Add background.service_worker field' };
  }
...
check('Manifest uses action (not browser_action)', () => {
  ...
  if (manifest.browser_action) {
    return { success: false, error: 'Uses deprecated browser_action', suggestion: 'Change to action' };
  }
  
  if (!manifest.action) {
    return { success: false, error: 'Missing action field', suggestion: 'Add action field for popup' };
  }
...
check('Manifest has proper V3 permissions', () => {
  ...
  const badPermissions = permissions.filter(p => 
    p === 'webRequest' || 
...
check('DNR rules use modifyHeaders action', () => {
  ...
  const nonModifyHeadersRules = rules.filter(rule => rule.action?.type !== 'modifyHeaders');
...
check('DNR rules cover major AI providers', () => {
  ...
  if (coveredProviders.length < expectedProviders.length) {
    const missing = expectedProviders.filter(p => !coveredProviders.includes(p));
    return { 
      success: false, 
