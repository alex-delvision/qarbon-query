...
test('Background script uses V3 APIs only', () => {
  const backgroundJs = fs.readFileSync(path.join(EXTENSION_DIR, 'background.js'), 'utf8');
  ...
  ].every(api => backgroundJs.includes(api));
...
test('DNR rules are properly structured', () => {
  const rules = JSON.parse(fs.readFileSync(path.join(EXTENSION_DIR, 'dnr_rules.json'), 'utf8'));
...
test('Popup script uses proper storage APIs', () => {
  ...
  return popupJs.includes('chrome.storage.local') &&
...
test('Scripts include proper error handling', () => {
  ...
  const hasErrorHandling = [backgroundJs, contentJs].every(script => 
    script.includes('try') && 
...
test('Supports all major AI providers', () => {
  const backgroundJs = fs.readFileSync(path.join(EXTENSION_DIR, 'background.js'), 'utf8');
  const dnrRules = JSON.parse(fs.readFileSync(path.join(EXTENSION_DIR, 'dnr_rules.json'), 'utf8'));
  
  const supportedProviders = ['openai', 'anthropic', 'gemini', 'bedrock', 'claude'];
...
if (testsFailed === 0) {
  ...
  console.log('✨ Extension is ready for Chrome with full Manifest V3 compliance.');
