...
class SSEAwareTracker {
  ...
  constructor() {
    ...
    console.log(
      '🌱 QarbonQuery SSE-Aware Tracker initialized for',
      this.platform
    );
  }
  ...
  private async handleJSONResponse(
    ...
    try {
      ...
      const tokens =
        data.usage?.total_tokens || Math.ceil(JSON.stringify(data).length / 4);
      const emissions = (tokens / 1000) * 0.002;
  ...
  private async storeCapture(capture: CaptureData): Promise<void> {
    ...
    console.log(
      `✅ Captured: ${capture.tokens} tokens, ${capture.emissions.toFixed(4)} g CO₂e`
    );
...
(window as any).__qarbonGetStats = () => {
  ...
  return {
    ...
    week,
    allTime: Object.values(stats).reduce(
      (sum: any, data: any) => ({
        captures: sum.captures + data.captures,
        tokens: sum.tokens + data.tokens,
        emissions: sum.emissions + data.emissions,
