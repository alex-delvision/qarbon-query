...
class TooltipTour {
  ...
  constructor() {
    this.addStep('#qarbon-tabs', 'Navigate through your carbon emission data using these tabs.', 'bottom');
    this.addStep('#qarbon-chart', 'This is your 14-day carbon emission trend chart.', 'bottom');
    this.addStep('[data-tooltip="settings"]', 'Access settings here to customize your experience.', 'left');
  }
  
  private addStep(element: string, text: string, position: 'top' | 'bottom' | 'left' | 'right'): void {
...
class QarbonStorageManager {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (period === 'week' || period === 'month') {
          ...
          const periodKey = period === 'week' ? this.getWeekKey(today) : this.getMonthKey(today);
  ...
  private static setCached(key: string, data: any, ttlMs: number = this.DEFAULT_TTL): void {
    this.cache.set(key, {
      data,
      timestamp: Date.now(),
      ttl: ttlMs
  ...
  static async getWeekEmissionsData(): Promise<{ total: number; breakdown: Record<string, number>; queries: number }> {
    return new Promise((resolve, reject) => {
      try {
        const today = new Date();
  ...
  static async getMonthEmissionsData(): Promise<{ total: number; breakdown: Record<string, number>; queries: number }> {
    return new Promise((resolve, reject) => {
      try {
        const today = new Date();
...
class PopupViewRouter {
  ...
  private async loadTodayData(): Promise<void> {
    ...
    this.updateEmissionsDisplayWithAnimation('emissions-total', aggregatedData.total);
    this.updatePlatformBreakdown('category-breakdown', aggregatedData.breakdown);
  ...
  private async loadWeekData(): Promise<void> {
    ...
    this.updateEmissionsDisplayWithAnimation('emissions-total-week', aggregatedData.total);
    this.updatePlatformBreakdown('category-breakdown-week', aggregatedData.breakdown);
    await renderTrendChart('week');
  }
  
  private async loadMonthData(): Promise<void> {
    const aggregatedData = await QarbonStorageManager.getAggregatedData('month');
    this.updateEmissionsDisplayWithAnimation('emissions-total-month', aggregatedData.total);
    this.updatePlatformBreakdown('category-breakdown-month', aggregatedData.breakdown);
    await renderTrendChart('month');
  }
  
  
  private updateEmissionsDisplayWithAnimation(elementId: string, totalEmissions: number): void {
  ...
  private updatePlatformBreakdown(elementId: string, breakdown: Record<string, { emissions: number; queries: number }>): void {
...
import { initializeUIEnhancements, loadDataWithSkeleton, renderEnhancedPlatformBreakdown, updateTabUnderline } from './ui-enhancements';

// Initialize UI enhancements
initializeUIEnhancements();

// Helper function for updating emissions display with animation
function updateEmissionsDisplayWithAnimation(element: HTMLElement, totalEmissions: number): void {
  // Auto-format between kg and g based on size
  const isKg = totalEmissions >= 1000;
  const displayValue = isKg ? totalEmissions / 1000 : totalEmissions;
  const unit = isKg ? 'kg' : 'g';
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadEmissionsDataForView(viewName: string): Promise<void> {
        ...
        const period = viewName === 'today' ? 'day' as const : 
                      viewName === 'week' ? 'week' as const :
                      viewName === 'month' ? 'month' as const : 'day' as const;
        
        const data = await loadDataWithSkeleton(() => QarbonStorageManager.getAggregatedData(period), {
            total: emissionsDisplay!,
            breakdown: categoryBreakdown!
        });
        updateEmissionsDisplayWithAnimation(emissionsDisplay!, data.total);
        await renderEnhancedPlatformBreakdown(`category-breakdown-${viewName}`, data.breakdown);
    }
 ...
 */
async function getDailyAggregates(period: 'day' | 'week' | 'month'): Promise<TrendDataPoint[]> {
    ...
    try {
      ...
      if (period === 'week' || period === 'month') {
        ...
        chrome.storage.local.get([chartKey], (result) => {
          ...
          if (chartData && chartData.labels && chartData.emissions && chartData.queries) {
            // Convert optimized chart data to TrendDataPoint format
            const trendData: TrendDataPoint[] = chartData.labels.map((_label: string, index: number) => {
 ...
 */
async function renderTrendChart(period: 'day' | 'week' | 'month'): Promise<void> {
  const canvasId = period === 'day' ? 'trend-chart' : 
                   period === 'week' ? 'trend-chart-week' : 
                   'trend-chart-month';
          ...
          tooltip: {
            ...
            callbacks: {
              ...
              label: function(context: any) {
                ...
                if (!point) return [];
                const emissionsKg = point.emissions >= 1000 ? 
