...
chrome.webRequest.onBeforeRequest.addListener(
  details => {
    ...
    if (
      details.url.includes('perplexity.ai') &&
...
chrome.webRequest.onBeforeRequest.addListener(
  details => {
    if (
      ...
      details.url.includes('perplexity')
    ) {
      ...
      chrome.storage.local.get(['emissions'], result => {
        ...
        chrome.storage.local.set({ emissions: data }, () => {
          console.log(
            `💚 Perplexity WebSocket tracked: ${emissions.toFixed(4)}g CO₂e`
          );
        });
      });
    }
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.type === 'SAVE_EMISSIONS') {
    chrome.storage.local.get(['emissions'], result => {
      ...
      chrome.storage.local.set({ emissions: data }, () => {
        console.log(
          '💾 Saved emissions from content script:',
          request.emissions
        );
