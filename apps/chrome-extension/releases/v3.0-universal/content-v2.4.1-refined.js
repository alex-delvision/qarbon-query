...
window.fetch = async function (...args) {
  ...
  if (window.location.hostname.includes('perplexity.ai')) {
    ...
    if (
      (url.includes('https://www.perplexity.ai/') || url.includes('wss://')) &&
      ...
      if (url.includes('www.perplexity.ai') && url.length > 50) {
        ...
        setTimeout(() => {
          ...
          console.log(
            `💚 Perplexity: ${emissions.toFixed(4)}g CO₂e (~${estimatedTokens} tokens)`
          );
          console.log(`📊 Total today: ${data[today].toFixed(4)}g`);

          // Reset flag after 10 seconds for next query
          setTimeout(() => {
            perplexityQueryTracked = false;
          }, 10000);
        }, 500);
      }
    }
...
console.log(
  '✅ QarbonQuery ready - All 4 platforms with better Perplexity handling'
);
